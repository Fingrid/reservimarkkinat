...
export async function validateXml(xmlContent: string): Promise<ValidationResult> {
    ...
    if (!state.isInitialized) {
        console.error("XML Schema Store not initialized before validation attempt.");
        return {
            isValid: false,
            details: [{ line: 0, message: "XML validation environment not ready. Please try again." }],
        };
    }
    ...
    try {
        ...
        return { isValid: true, schema: { url: xmlItem.fileUrl, urn: xmlItem.urn } };
        ...
        const xmlItem = state.getSchemaByUrn(xmlNamespace);
        const returnSchema = xmlItem?.fileUrl && xmlItem.urn ? { url: xmlItem.fileUrl, urn: xmlItem.urn } : undefined;


        if (err instanceof XmlValidateError) {
            return {
                isValid: false,
                details: err.details.map(d => ({ line: d.line ?? 0, message: d.message })),
