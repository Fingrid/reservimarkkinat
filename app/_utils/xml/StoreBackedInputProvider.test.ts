...
const createMockStoreProvider = (schemas: Map<string, XMLBufferItem>): SchemaStoreProvider => ({
    getSchemaByUrn: vi.fn((urn) => Array.from(schemas.values()).find(item => item.urn === urn)),
...
describe('StoreBackedInputProvider', () => {
    ...
    beforeEach(() => {
        ...
        mockSchemas = new Map<string, XMLBufferItem>([
            ['exists.xsd', { urn: 'urn:exists', fileUrl: '/path/exists.xsd', buffer: Buffer.from('exists content') }],
            ['no_buffer.xsd', { urn: 'urn:no_buffer', fileUrl: '/path/no_buffer.xsd', buffer: undefined as unknown as Buffer<ArrayBufferLike>}], // Simulate missing buffer
        ]);
    ...
    describe('match', () => {
        it('should return true if the store has the schema by filename', () => {
            ...
            expect(mockStoreProvider.hasSchema).toHaveBeenCalledWith({ key: 'exists.xsd', type: 'filename' });
        });
        ...
        it('should return false if the store does not have the schema by filename', () => {
            ...
            expect(mockStoreProvider.hasSchema).toHaveBeenCalledWith({ key: 'does_not_exist.xsd', type: 'filename' });
        });
    });
    ...
    describe('open', () => {
        it('should get the schema by filename and call openBuffer with the buffer', () => {
            ...
            expect(mockStoreProvider.getSchemaByFilename).toHaveBeenCalledWith('exists.xsd');
            expect(openBuffer).toHaveBeenCalledWith(mockSchemas.get('exists.xsd')?.buffer);
        ...
        it('should throw an error if the schema is not found by filename', () => {
            ...
            expect(mockStoreProvider.getSchemaByFilename).toHaveBeenCalledWith('does_not_exist.xsd');
        ...
        it('should throw an error if the schema buffer is missing', () => {
            ...
            expect(mockStoreProvider.getSchemaByFilename).toHaveBeenCalledWith('no_buffer.xsd');
            expect(openBuffer).not.toHaveBeenCalled();
        });
    });
